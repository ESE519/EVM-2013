...
int createNextTaskPacket(char *txBuffer,uint8_t *data,int dataSize)
{
	...
	memcpy(&txBuffer[DATA_START_LOCATION],data,MAX_PACKET_SIZE);//Copy data into the transmit buffer
	...
	else if(packetNumber == numberOfPackets)
	{
		...
		memcpy(&txBuffer[DATA_START_LOCATION],data,(dataSize%MAX_PACKET_SIZE)); ////Copy data into the transmit buffer with right size
		firstPacket=1;
		packetNumber = 0; 
