...
int8_t nrk_TCB_init (nrk_task_type *Task, NRK_STK *ptos, NRK_STK *pbos, uint16_t stk_size, void *pext, uint16_t opt)
{
    ...
    nrk_task_TCB[Task->task_ID].period= _nrk_time_to_ticks( &(Task->period) );
		...
		//Added by Sumukh . Initializing the relative_deadline parameter of the task 
		if(Task->period.secs == 0 && Task->period.nano_secs ==0)
		{
			Task->relative_deadline.secs = Task->period.secs;
			Task->relative_deadline.nano_secs = Task->period.nano_secs;
		}
		
		nrk_task_TCB[Task->task_ID].relative_deadline = _nrk_time_to_ticks(&(Task->relative_deadline));
		//
